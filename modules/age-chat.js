...
export async function chatDamageRoll(event) {
    ...
    if (!owner) return ui.notifications.warn(game.i18n.localize("the-expanse.WARNING.originTokenMissing"));
...
export async function sortCustomAgeChatCards(chatCard, html, data) {
    ...
    _buttonType(html.find(".the-expanse.item-chat-controls button"));
...
async function _handleAgeRollVisibility(html, chatCard, chatData){
    const element = html.find(".the-expanse.base-age-roll .feature-controls");
...
function _permCheck(actorPerm, element) {
    ...
    if (game.settings.get("the-expanse", "observerRoll")) validPerm.push(CONST.ENTITY_PERMISSIONS.OBSERVER);
