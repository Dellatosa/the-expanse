export class AgeTracker extends Application {
	...
	get template() {
		return `systems/the-expanse/templates/age-tracker.hbs`;
	}
	
	getData(options) {
		const data = super.getData(options);
		data.isGM = game.user.isGM;
		data.colorset = game.settings.get("the-expanse", "colorScheme");

		data.hasSerendipity = game.settings.get("the-expanse", "serendipity");
		if (data.hasSerendipity) {
			const serData = game.settings.get("the-expanse", "serendipityValue");
			if (serData.actual > serData.max) {
				serData.actual = serData.max;
				game.settings.set("the-expanse", "serendipityValue", serData);
			};
			const sides = 6;
		...
		const compType = game.settings.get("the-expanse", "complication");
		data.hasComplication = (compType === "none") ? false : true;
		if (data.hasComplication) {
			const compData = game.settings.get("the-expanse", "complicationValue");
	...
	activateListeners(html) {
		...
		const trackerPos = game.user.getFlag("the-expanse", "ageTrackerPos");
	...
	_onRightClick(event) {
		...
		game.user.setFlag("the-expanse", "ageTrackerPos", original);
	}

	_onClickComp(event) {
		event.preventDefault();
		const compData = game.settings.get("the-expanse", "complicationValue");
		if (event.currentTarget.classList.contains('refresh')) return game.settings.set("the-expanse", "complicationValue", {max: compData.max, actual: 0});
		let value;
		...
		game.settings.set("the-expanse", "complicationValue", compData);
	}
	...
	_dragElement(elmnt) {
		...
		function closeDragElement() {
			...
			game.user.setFlag("the-expanse", "ageTrackerPos", trackerPos);
		}
	}
}