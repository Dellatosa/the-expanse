...
export class ageSystemActor extends Actor {
    ...
    prepareData() {
        ...
        if (this.data.type === 'char') {
            ...
            const initiativeFocus = game.settings.get("the-expanse", "initiativeFocus");
            if (initiativeFocus) {
    ...
    prepareBaseData() {
        ...
        data.useBallisticArmor = game.settings.get("the-expanse", "useBallisticArmor");

        // Retrieve wealth mode
        data.useResource = data.useIncome = data.useCurrency = data.useCoins = false;
        const wealthMode = game.settings.get("the-expanse", "wealthType");
        switch (wealthMode) {
            case "income":
                data.useIncome = true;
                break;
    ...
    //         const gmScreenFlag = {gmScreenSheetClass: `the-expanse.ageSystemSheetCharStatBlock`};
    //         this.data.flags = {
    //             ...this.data.flgas,
    ...
    _prepareBaseDataChar() {
        ...
        data.useConviction = game.settings.get("the-expanse", "useConviction");

        // Check if Toughness is in use
        data.useToughness = game.settings.get("the-expanse", "useToughness");

        // Check if Fatigue is in use
        data.useFatigue = game.settings.get("the-expanse", "useFatigue");

        // Check if Power Points is in use
        data.usePowerPoints = game.settings.get("the-expanse", "usePowerPoints");
    ...
    handleConditions(condId, isChecked = null) {
        ...
        const condEffects = this.effects.filter(c => c.data.flags?.["the-expanse"]?.type === "conditions" && c.data.flags?.["the-expanse"]?.name === condId);
        ...
        if (isChecked && condEffects.length < 1) {
            const newEffect = CONFIG.statusEffects.filter(e => e.flags?.["the-expanse"]?.name === condId)[0];
