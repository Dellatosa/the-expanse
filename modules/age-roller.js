...
export class AgeRoller extends Application {
	...
	get template() {
		return `systems/the-expanse/templates/rolls/age-roller.hbs`;
	}
	...
	activateListeners(html) {
		...
		const rollerPos = game.user.getFlag("the-expanse", "ageRollerPos");
	...
	refresh() {
 	...
 	async _onClick(event) {
		...
		const rollHeader = game.i18n.format("the-expanse.chatCard.looseRoll", {user});
	...
	_onResetPosition(event) {
		...
		game.user.setFlag("the-expanse", "ageRollerPos", original);
	}
	...
	_dragElement(elmnt) {
		...
		function closeDragElement() {
			...
			game.user.setFlag("the-expanse", "ageRollerPos", rollerPos);
		}
	}
}