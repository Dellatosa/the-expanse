...
export default class ageSystemItemSheet extends ItemSheet {
    ...
    static get defaultOptions() {        
        ...
        return mergeObject(super.defaultOptions, {
            height: 450,
            width: 500,
            classes: ["the-expanse", "sheet", "item", "colorset-second-tier"],
            tabs: [{
    ...
    get template() {
        return `systems/the-expanse/templates/sheets/${this.item.data.type}-sheet.hbs`;
    };
    ...
    getData(options) {
        ...
        data.config.wealthMode = game.settings.get("the-expanse", "wealthType");
        ...
        if (this.item.data.type === "shipfeatures") {
            ...
            for (let f = 0; f < data.config.featuresType.length; f++) {
                ...
                data.config.featuresTypeLocal.push({
                    key: feat,
                    name: game.i18n.localize(`the-expanse.spaceship.${feat}`)
                });
            }
    ...
    async _onAddBonus(event) {
        ...
        for (const mod in bonusList) {
            if (Object.hasOwnProperty.call(bonusList, mod)) {
                ...
                if (!b.selected) {
                    const modName = game.i18n.localize(`the-expanse.bonus.${mod}`);
        ...
        const template = `systems/the-expanse/templates/bonus-select-form.hbs`;
        ...
        return new Promise(resolve => {
            ...
            new Dialog(data, {classes: ["the-expanse-dialog", "bonus-select", "dialog"]}).render(true);
        });
    }
