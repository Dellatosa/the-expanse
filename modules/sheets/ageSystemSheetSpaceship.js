...
export default class ageSpaceshipSheet extends ActorSheet {
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            height: 750,
            resizable: false,
            classes: ["the-expanse", "sheet", "spaceship"]
        });
    }
                ...
                if (passenger.data.type === "char") {
                    ...
                    passengerList.map( p => {
                        if (p.id === passengerData.id) {
                            ...
                            let warning = game.i18n.format("the-expanse.WARNING.alreadyOnboard", parts);
                    ...
                    const warning = game.i18n.localize("the-expanse.WARNING.vehicleIsNotPassenger");
                    ui.notifications.warn(warning);
                }
            })
        };
    ...
    _onRollDice(event){
        ...
        if (event.currentTarget.classList.contains("roll-hull")) {
            ...
            messageData.flavor += ` | ${game.i18n.localize("the-expanse.spaceship.hull")}`;
        }
    ...
    _onRollManeuver(event) {
        ...
        if (datum.passengerId === "crew") {
            passenger.name = game.i18n.localize("the-expanse.spaceship.crew");
            const crewAction = {
            ...
            if (!passenger) {
                ...
                let warning = game.i18n.format("the-expanse.WARNING.userNotAvailable", parts);
        ...
        let flavorText = "the-expanse.chatCard.maneuversVehicle";
        switch (datum.sysName) {
            case "command":
                flavorText = "the-expanse.chatCard.commandsVehicle";
                break;
            case "sensors":
                flavorText = "the-expanse.chatCard.operatesVehicle";
                break;
            case "damageControl":
                flavorText = "the-expanse.chatCard.backupVehicle";
                break;
            default:
                break;
        }
        ...
        if (system) {
            rollData.moreParts.push({
                value: system.total,
                description: game.i18n.localize(`the-expanse.spaceship.systemName.${datum.sysName}`)
            })
        }
