...
export default class ageSystemSheetCharacter extends ActorSheet {
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            width: 680,
            height: 800,
            classes: ["the-expanse", "sheet", "char", "standard"],
            tabs: [{
    ...
    get template() {
        return `systems/the-expanse/templates/sheets/${this.actor.data.type}-sheet.hbs`;
    }

    get observerRoll () {
        return game.settings.get("the-expanse", "observerRoll");
    }
    ...
    getData(options) {
        ...
        data.condEffects = data.effects.filter(e => e.flags?.["the-expanse"]?.type === "conditions");
        // ...from all other Effects
        data.effects = data.effects.filter(e => !e.flags?.["the-expanse"]?.type);
        data.effects = sortObjArrayByName(data.effects, `label`);
    
        // Retrieve Prefession/Ancestry settings
        data.ancestry = game.settings.get("the-expanse", "ancestryOpt");
        data.occupation = game.settings.get("the-expanse", "occupation");

        // Retrieve Health Mode (Health/Fortune)
        data.healthMode = game.settings.get("the-expanse", "healthMode");

        // Sheet color
        data.colorScheme = game.settings.get("the-expanse", "colorScheme");
    ...
    _getHeaderButtons() {
        ...
        const isFull = sheet === undefined || sheet === 'the-expanse.ageSystemSheetCharacter';

        buttons = [
            {
              label: isFull ? game.i18n.localize("the-expanse.sheetBlock") : game.i18n.localize("the-expanse.sheetFull"),
    ...
    async _onToggleSheet(event) {
        ...
        let newSheet = 'the-expanse.ageSystemSheetCharStatBlock'
        ...
        if (original != 'the-expanse.ageSystemSheetCharacter') newSheet = 'the-expanse.ageSystemSheetCharacter'
    ...
    _onAddEffect(event) {
        const newEffect = {
            label: game.i18n.localize("the-expanse.item.newItem"),
    ...
    focusContextMenu = [
        {
            name: game.i18n.localize("the-expanse.ageRollOptions"),
        ...
        {
            name: game.i18n.localize("the-expanse.chatCard.roll"),
        ...
        {
            name: game.i18n.localize("the-expanse.settings.changeRollContext"),
        ...
        {
            name: game.i18n.localize("the-expanse.settings.edit"),
        ...
        {
            name: game.i18n.localize("the-expanse.settings.delete"),
